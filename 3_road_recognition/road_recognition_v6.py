...
# 在v5的基础上新增&修改，希望能够实现由定时器控制LED灯闪烁
...
tim_blue = Timer(3, freq=4)      # 使用定时器3创建定时器对象—以4Hz触发
...
while(True):
    ...
    if((x_sum/total_points)>50): # 中心线在图像中偏右
        # 蓝灯闪烁
        tim_red.callback(None)
        tim_blue.callback(blue_LED_flash)          # 将回调设置为blue_LED_flash函数
        red_led.off()
        green_led.off()
        # blue_led.on()
    elif((x_sum/total_points)<30): # 中心线在图像中偏左
        # 红灯闪烁
        tim_blue.callback(None)
        tim_red.callback(red_LED_flash)          # 将回调设置为red_LED_flash函数
        # red_led.on()
        green_led.off()
        blue_led.off()
    else: # 不偏左也不偏右
